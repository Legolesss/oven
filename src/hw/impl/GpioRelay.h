...
#include <gpiod.h>
#include <stdexcept>
#include "../IRelay.h"   // must define set(bool), get() const

class GpioRelay final : public IRelay {
  gpiod_chip* chip_;
  gpiod_line* line_;
  bool state_{false};
  bool activeHigh_;
public:
  GpioRelay(const char* chipName, unsigned lineOffset, bool activeHigh = true)
