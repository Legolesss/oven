...
#include <gpiod.h>
#include <stdexcept>
#include "../IHeater.h"   // must define set(bool), get() const

class GpioHeater final : public IHeater {
  gpiod_chip* chip_;
  gpiod_line* line_;
  bool state_{false};
  bool activeHigh_;
public:
  GpioHeater(const char* chipName, unsigned lineOffset, bool activeHigh = true)
